...
class CircleChanger(object):
    ...
    def get_distance_from(self, point):
        ...
        ################################################################
        answer = math.sqrt((self.circle.center.x-point.x)**2+(self.circle.center.y-point.y)**2)
        return answer
    ...
    def swell_or_shrink_once(self, amount_to_swell_or_shrink):
        ...
        ################################################################
        if (-1 * amount_to_swell_or_shrink) <= self.circle.radius - 1:
    ...
    def swallow(self, other_circle_changer):
        ...
        #   NO CREDIT if you use the distance formula here.
        ################################################################
        holder = self.circle.center.halfway_to(other_circle_changer.circle.center)
        colorholder = self.colors+other_circle_changer.colors
        rdus = self.circle.center.get_distance_from(other_circle_changer.circle.center)/2
        kirby = CircleChanger(holder.x, holder.y, rdus, 'red', colorholder)
        return kirby
    ...
    def change_to_next_color_in_tuple(self):
        ...
        ################################################################
        self.circle.fill_color=self.colors[self.listed]
        self.listed = (self.listed+1) % len(self.colors)
